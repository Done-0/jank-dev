...
import { Search, Plus, MoreHorizontal, Eye, Trash2, Settings, Shield, UserPlus, Crown } from 'lucide-react';
...
export function RBACMainContent({ selectedSection, searchTerm, onSearchChange }: RBACMainContentProps) {
  ...
  const [isViewRolePermissionsOpen, setIsViewRolePermissionsOpen] = useState(false);
  ...
  const permissions = useMemo(() => permissionsResponse?.list || [], [permissionsResponse]);
  ...
  const handleCreateRole = () => {
    if (!newRoleName.trim() || !newRoleResource.trim() || !newRoleAction.trim()) {
  ...
  const handleCreatePermission = () => {
    if (!newPermissionRole.trim() || !newPermissionResource.trim() || !newPermissionAction.trim()) {
  ...
  const handleAssignPermission = () => {
    if (!assignPermissionRole.trim() || !assignPermissionResource.trim() || !assignPermissionAction.trim()) {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-4 border-b">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button onClick={() => setIsCreateRoleOpen(true)} className="h-10 px-4 shrink-0 rounded-full">
          ...
          <div className="flex-1 overflow-auto p-4">
            <div className="text-center py-12 border-2 border-dashed border-muted-foreground/30 rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">
                <span className="hidden sm:inline">点击上方&ldquo;创建角色&rdquo;按钮开始创建</span>
          ...
          <div className="flex-1 overflow-y-scroll scrollbar-hidden">
            <div className="divide-y divide-border">
              {filteredRoles.map((role: string, index: number) => (
                <div key={index} className="px-4 py-4 hover:bg-accent/50 transition-colors">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-medium text-lg line-clamp-2 flex-1 min-w-0">{role}</h3>
                    ...
                    <div className="flex items-center justify-end gap-3">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          <DropdownMenuItem onClick={() => handleViewRolePermissions(role)} className="cursor-pointer">
                            <Eye className="mr-2 h-4 w-4" />
                            查看权限
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleDeleteRole(role)} className="cursor-pointer text-destructive focus:text-destructive">
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-4 border-b">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button onClick={() => setIsCreatePermissionOpen(true)} className="h-10 px-4 shrink-0 rounded-full">
            <Plus className="mr-2 h-4 w-4" />
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-start justify-between gap-3">
                        <h3 className="font-medium text-lg line-clamp-2 flex-1 min-w-0">
                          {permission.name || resourceInfo?.name || permission.resource}
                        </h3>
                        <Badge variant="secondary" className="shrink-0">
                          {permission.action}
                        </Badge>
                      ...
                      <div className="min-h-[1.25rem]">
                        <p className="text-sm text-muted-foreground line-clamp-2">
                          {permission.description || resourceInfo?.description || permission.resource}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-40">
                            <DropdownMenuItem onClick={handlePermissionInfo} className="cursor-pointer">
                              <Eye className="mr-2 h-4 w-4" />
                              查看详情
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={handleDeletePermission} className="cursor-pointer text-destructive focus:text-destructive">
                              <Trash2 className="mr-2 h-4 w-4" />
                              删除权限
                            </DropdownMenuItem>
                          </DropdownMenuContent>
        ...
        {rolesLoading ? (
          ...
          <div className="flex-1 overflow-auto p-4">
            <div className="text-center py-12 border-2 border-dashed border-muted-foreground/30 rounded-lg">
              ...
              <p className="text-muted-foreground mb-4">暂无任何角色数据</p>
              <p className="text-sm text-muted-foreground">
                <span className="hidden sm:inline">请先在角色管理中创建角色</span>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-lg line-clamp-2">{user.nickname}</h3>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          <DropdownMenuItem 
                            onClick={() => toast.info(`管理用户 ${user.nickname} 的角色功能开发中...`)} 
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem 
                            onClick={() => toast.info(`查看用户 ${user.nickname} 详情功能开发中...`)} 
  ...
  return (
    <div className="h-full">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="roleDescription">描述 <span className="text-muted-foreground">(可选)</span></Label>
                <Input
                  id="roleDescription"
            ...
            <div className="border-t pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={newRoleResource} onValueChange={setNewRoleResource}>
                    <SelectTrigger>
                      ...
                      {RESOURCE_GROUPS.map((group) => (
                        <div key={group.key}>
                          ...
                          {group.resources.map((resource) => (
                            <SelectItem key={resource.value} value={resource.value} className="pl-4">
                ...
                <div>
                  ...
                  <Select value={newRoleAction} onValueChange={setNewRoleAction}>
                    <SelectTrigger>
                      <SelectValue placeholder="选择操作" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value={RBAC_ACTION.GET}>{RBAC_ACTION.GET}</SelectItem>
                      <SelectItem value={RBAC_ACTION.POST}>{RBAC_ACTION.POST}</SelectItem>
                      <SelectItem value={RBAC_ACTION.ALL}>{RBAC_ACTION.ALL}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={createRoleMutation.isPending || !newRoleName.trim() || !newRoleResource || !newRoleAction}
            >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={newPermissionRole} onValueChange={setNewPermissionRole}>
                <SelectTrigger>
      ...
      <Dialog open={isAssignRoleOpen} onOpenChange={setIsAssignRoleOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAssignRoleDialog}>
              分配
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={assignPermissionRole} onValueChange={setAssignPermissionRole}>
                <SelectTrigger>
                  <SelectValue placeholder="选择角色" />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Select value={assignPermissionResource} onValueChange={setAssignPermissionResource}>
                <SelectTrigger>
                  <SelectValue placeholder="选择资源" />
                </SelectTrigger>
                <SelectContent className="max-h-80">
            ...
            <div>
              ...
              <Select value={assignPermissionAction} onValueChange={setAssignPermissionAction}>
                <SelectTrigger>
                  <SelectValue placeholder="选择操作" />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAssignPermissionOpen(false)}>
              取消
            </Button>
            <Button onClick={handleAssignPermission} disabled={assignPermissionMutation.isPending}>
      ...
      {/* 查看角色权限对话框 */}
      <Dialog open={isViewRolePermissionsOpen} onOpenChange={setIsViewRolePermissionsOpen}>
          ...
          <div className="space-y-4">
            {rolePermissionsLoading ? (
              ...
              <div className="text-center py-8 border-2 border-dashed border-muted-foreground/30 rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">该角色还没有分配任何权限</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {rolePermissions.map((permission: any, index: number) => {
                  ...
                  return (
                    <div key={index} className="px-4 py-4 hover:bg-accent/50 transition-colors border rounded-lg">
                      <div className="flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-3">
