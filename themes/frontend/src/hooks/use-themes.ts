...
export function useActiveTheme(
  options?: Omit<UseQueryOptions<GetActiveThemeResponse>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
...
export function useSwitchTheme(
  options?: Omit<UseMutationOptions<SwitchThemeResponse, Error, SwitchThemeRequest>, 'mutationFn'>
) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (request: SwitchThemeRequest) => themeService.switchTheme(request),
    onSuccess: (data) => {
